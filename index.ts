...
(async () => {
  const res = await fetch("http://www.call4papers.cn/ccf/ccf-1.jsp").then(res =>
    res.text()
  );
  const {
    window: { document }
  ...
  for (const [index, category] of categories.entries()) {
    ...
    if (index !== 0) {
      const res = await fetch(`http://www.call4papers.cn/ccf/${href}`).then(
        res => res.text()
      );
      const {
        window: { document }
  ...
  writeFile("output.json", JSON.stringify(output, null, 2), () =>
